...
fn append_log(settings: &SyncSettings, message: &str) {
    ...
    if let Ok(mut file) = OpenOptions::new().create(true).append(true).open(&settings.log_file) {
...
fn run_sync_cycle(state: &AppState) -> Result<SyncReport, String> {
    ...
    match sync_all(&settings.codex_home, &backup_dir, &settings.providers, true) {
        ...
        Err(err) => {
            let mut report = state.last_report.lock().map_err(|lock_err| lock_err.to_string())?.clone().unwrap_or_default();
            report.last_error = Some(err.to_string());
            append_log(&settings, &format!("sync failed: {err}"));
            *state.last_report.lock().map_err(|lock_err| lock_err.to_string())? = Some(report);
            Err(err.to_string())
        }
    }
}
...
fn start_background(app: AppHandle) {
    ...
    thread::spawn(move || loop {
        ...
        let interval = settings.interval_seconds.max(settings::MIN_INTERVAL_SECONDS);
...
fn run_key() -> Result<RegKey, String> {
    ...
    hkcu.create_subkey(path).map(|(key, _)| key).map_err(|err| err.to_string())
}

#[cfg(target_os = "windows")]
fn autostart_enabled_inner() -> bool {
    run_key().and_then(|key| key.get_value::<String, _>(RUN_KEY_NAME).map_err(|err| err.to_string())).is_ok()
}
...
fn set_autostart_inner(enabled: bool) -> Result<bool, String> {
    ...
    if enabled {
        ...
        key.set_value(RUN_KEY_NAME, &format!("\"{}\"", exe.display())).map_err(|err| err.to_string())?;
        Ok(true)
    } else {
...
fn get_last_report(state: State<'_, AppState>) -> Option<SyncReport> {
    state.last_report.lock().ok().and_then(|report| report.clone())
}
...
#[tauri::command]
fn set_interval(interval_seconds: u64) -> Result<SyncSettings, String> {
